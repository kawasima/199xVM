...
impl Vm {
    ...
    pub(crate) fn execute_opcode(
        ...
        match opcode {
            ...
            0x1a..=0x1d => {
                let i = (opcode - 0x1a) as usize;
                frame.stack.push(frame.locals[i].clone());
            } // iload_0..3
            0x1e..=0x21 => {
                let i = (opcode - 0x1e) as usize;
                frame.stack.push(frame.locals[i].clone());
            } // lload_0..3
            0x22..=0x25 => {
                let i = (opcode - 0x22) as usize;
                frame.stack.push(frame.locals[i].clone());
            } // fload_0..3
            0x26..=0x29 => {
                let i = (opcode - 0x26) as usize;
                frame.stack.push(frame.locals[i].clone());
            } // dload_0..3
            0x2a..=0x2d => {
                let i = (opcode - 0x2a) as usize;
                frame.stack.push(frame.locals[i].clone());
            } // aload_0..3
                ...
                if let Some(r) = arr_ref.as_ref() {
                    let elem = match &r.borrow().native {
                        NativePayload::Array(v) => {
                            v.get(idx).cloned().ok_or_else(|| array_oob(idx_i))?
                        }
                        _ => JValue::Ref(None),
                    };
                ...
                if let Some(r) = arr_ref.as_ref() {
                    let elem = match &r.borrow().native {
                        NativePayload::ByteArray(v) => {
                            JValue::Int(*v.get(idx).ok_or_else(|| array_oob(idx_i))? as i32)
                        }
                ...
                if let Some(r) = arr_ref.as_ref() {
                    let elem = match &r.borrow().native {
                        NativePayload::IntArray(v) => {
                            JValue::Int(*v.get(idx).ok_or_else(|| array_oob(idx_i))?)
                        }
                        NativePayload::Array(v) => {
                            v.get(idx).ok_or_else(|| array_oob(idx_i))?.clone()
                        }
                        _ => JValue::Int(0),
                    };
                    frame.stack.push(elem);
                ...
                if let Some(r) = arr_ref.as_ref() {
                    let elem = match &r.borrow().native {
                        ...
                        NativePayload::LongArray(v) => {
                            JValue::Long(*v.get(idx).ok_or_else(|| array_oob(idx_i))?)
                        }
                        NativePayload::IntArray(v) => {
                            JValue::Int(*v.get(idx).ok_or_else(|| array_oob(idx_i))?)
                        }
            ...
            0x3b..=0x3e => {
                let i = (opcode - 0x3b) as usize;
                let v = frame.stack.pop().unwrap();
                frame.locals[i] = v;
            } // istore_0..3
            0x3f..=0x42 => {
                let i = (opcode - 0x3f) as usize;
                let v = frame.stack.pop().unwrap();
                frame.locals[i] = v;
            } // lstore_0..3
            0x43..=0x46 => {
                let i = (opcode - 0x43) as usize;
                let v = frame.stack.pop().unwrap();
                frame.locals[i] = v;
            } // fstore_0..3
            0x47..=0x4a => {
                let i = (opcode - 0x47) as usize;
                let v = frame.stack.pop().unwrap();
                frame.locals[i] = v;
            } // dstore_0..3
            0x4b..=0x4e => {
                let i = (opcode - 0x4b) as usize;
                let v = frame.stack.pop().unwrap();
                frame.locals[i] = v;
            } // astore_0..3
            ...
            0x55 => {
                // castore (char array store — treated same as iastore)
                let val = frame.stack.pop().unwrap();
                let idx_i = frame.stack.pop().unwrap().as_int();
                let arr_ref = frame.stack.pop().unwrap();
                let idx = array_index(idx_i)?;
            ...
            0x50 | 0x51 | 0x52 | 0x56 => {
                // lastore, fastore, dastore, sastore
                let val = frame.stack.pop().unwrap();
                let idx_i = frame.stack.pop().unwrap().as_int();
                let arr_ref = frame.stack.pop().unwrap();
                let idx = array_index(idx_i)?;
            ...
            0x54 => {
                ...
                match arr_ref.as_ref() {
                    ...
                    Some(r) => match r.borrow_mut().native {
                        ...
                        NativePayload::Array(ref mut v) => {
                            *v.get_mut(idx).ok_or_else(|| array_oob(idx_i))? =
                                JValue::Int(val as i32);
                        }
                        _ => {}
                    },
                }
            ...
            0x60 => {
                ...
                frame.stack.push(JValue::Int(a.wrapping_add(b)));
            } // iadd
            0x64 => {
                let b = frame.stack.pop().unwrap().as_int();
                let a = frame.stack.pop().unwrap().as_int();
                frame.stack.push(JValue::Int(a.wrapping_sub(b)));
            } // isub
            0x68 => {
                let b = frame.stack.pop().unwrap().as_int();
                let a = frame.stack.pop().unwrap().as_int();
                frame.stack.push(JValue::Int(a.wrapping_mul(b)));
            } // imul
            0x6c => {
                // idiv
                let b = frame.stack.pop().unwrap().as_int();
            ...
            0x74 => {
                let a = frame.stack.pop().unwrap().as_int();
                frame.stack.push(JValue::Int(a.wrapping_neg()));
            } // ineg
            0x7e => {
                let b = frame.stack.pop().unwrap().as_int();
                let a = frame.stack.pop().unwrap().as_int();
                frame.stack.push(JValue::Int(a & b));
            } // iand
            0x80 => {
                let b = frame.stack.pop().unwrap().as_int();
                let a = frame.stack.pop().unwrap().as_int();
            ...
            0x78 => {
                let b = frame.stack.pop().unwrap().as_int() & 0x1f;
                let a = frame.stack.pop().unwrap().as_int();
                frame.stack.push(JValue::Int(a << b));
            } // ishl
            0x7a => {
            ...
            0x7c => {
                let b = frame.stack.pop().unwrap().as_int() & 0x1f;
                let a = frame.stack.pop().unwrap().as_int();
                frame.stack.push(JValue::Int(((a as u32) >> b) as i32));
            } // iushr
            ...
            0x61 => {
                ...
                frame.stack.push(JValue::Long(a.wrapping_add(b)));
            } // ladd
            0x65 => {
                let b = frame.stack.pop().unwrap().as_long();
                let a = frame.stack.pop().unwrap().as_long();
                frame.stack.push(JValue::Long(a.wrapping_sub(b)));
            } // lsub
            0x69 => {
                let b = frame.stack.pop().unwrap().as_long();
                let a = frame.stack.pop().unwrap().as_long();
                frame.stack.push(JValue::Long(a.wrapping_mul(b)));
            } // lmul
            0x6d => {
                // ldiv
                let b = frame.stack.pop().unwrap().as_long();
            ...
            0x75 => {
                let a = frame.stack.pop().unwrap().as_long();
                frame.stack.push(JValue::Long(a.wrapping_neg()));
            } // lneg
            0x7f => {
                let b = frame.stack.pop().unwrap().as_long();
                let a = frame.stack.pop().unwrap().as_long();
                frame.stack.push(JValue::Long(a & b));
            } // land
            0x81 => {
                let b = frame.stack.pop().unwrap().as_long();
                let a = frame.stack.pop().unwrap().as_long();
            ...
            0x79 => {
                let b = frame.stack.pop().unwrap().as_int() & 0x3f;
                let a = frame.stack.pop().unwrap().as_long();
                frame.stack.push(JValue::Long(a << b));
            } // lshl
            0x7b => {
            ...
            0x7d => {
                let b = frame.stack.pop().unwrap().as_int() & 0x3f;
                let a = frame.stack.pop().unwrap().as_long();
                frame.stack.push(JValue::Long(((a as u64) >> b) as i64));
            } // lushr
            0x94 => {
                // lcmp
                let b = frame.stack.pop().unwrap().as_long();
            ...
            0x62 => {
                ...
                frame.stack.push(JValue::Float(a + b));
            } // fadd
            0x66 => {
                let b = frame.stack.pop().unwrap().as_float();
                let a = frame.stack.pop().unwrap().as_float();
            ...
            0x76 => {
                let a = frame.stack.pop().unwrap().as_float();
                frame.stack.push(JValue::Float(-a));
            } // fneg
            0x95 => {
                // fcmpl (NaN → -1)
                let b = frame.stack.pop().unwrap().as_float();
                ...
                frame.stack.push(JValue::Int(if a > b {
                    1
                } else if a == b {
                    0
                } else if a < b {
            ...
            0x63 => {
                ...
                frame.stack.push(JValue::Double(a + b));
            } // dadd
            0x67 => {
                let b = frame.stack.pop().unwrap().as_double();
                let a = frame.stack.pop().unwrap().as_double();
            ...
            0x77 => {
                let a = frame.stack.pop().unwrap().as_double();
                frame.stack.push(JValue::Double(-a));
            } // dneg
            0x97 => {
                // dcmpl (NaN → -1)
                let b = frame.stack.pop().unwrap().as_double();
            ...
            0x85 => {
                ...
                frame.stack.push(JValue::Long(v as i64));
            } // i2l
            0x86 => {
                let v = frame.stack.pop().unwrap().as_int();
                frame.stack.push(JValue::Float(v as f32));
            } // i2f
            0x87 => {
                let v = frame.stack.pop().unwrap().as_int();
                frame.stack.push(JValue::Double(v as f64));
            } // i2d
            0x88 => {
                let v = frame.stack.pop().unwrap().as_long() as i32;
                frame.stack.push(JValue::Int(v));
            } // l2i
            0x89 => {
                let v = frame.stack.pop().unwrap().as_long();
                frame.stack.push(JValue::Float(v as f32));
            } // l2f
            0x8a => {
                let v = frame.stack.pop().unwrap().as_long();
            ...
            0x8b => {
                let v = frame.stack.pop().unwrap().as_float();
                frame.stack.push(JValue::Int(float_to_int(v)));
            } // f2i
            0x8c => {
                let v = frame.stack.pop().unwrap().as_float();
                frame.stack.push(JValue::Long(float_to_long(v)));
            } // f2l
            0x8d => {
                let v = frame.stack.pop().unwrap().as_float();
                frame.stack.push(JValue::Double(v as f64));
            ...
            0x8e => {
                let v = frame.stack.pop().unwrap().as_double();
                frame.stack.push(JValue::Int(double_to_int(v)));
            } // d2i
            0x8f => {
                let v = frame.stack.pop().unwrap().as_double();
                frame.stack.push(JValue::Long(double_to_long(v)));
            } // d2l
            0x90 => {
                let v = frame.stack.pop().unwrap().as_double();
                frame.stack.push(JValue::Float(v as f32));
            ...
            0x91 => {
                let v = frame.stack.pop().unwrap().as_int() as i8;
                frame.stack.push(JValue::Int(v as i32));
            } // i2b
            0x92 => {
                let v = frame.stack.pop().unwrap().as_int() as u16;
                frame.stack.push(JValue::Int(v as i32));
            } // i2c
            0x93 => {
                let v = frame.stack.pop().unwrap().as_int() as i16;
                frame.stack.push(JValue::Int(v as i32));
            } // i2s
            ...
            0xb2 => {
                ...
                let field = self.resolve_fieldref_cached(cp, idx);
                let class_name = field.class_name.as_str();
                if self.ensure_class_init_or_schedule(class_name)? {
                    frame.pc = opcode_pc;
                    return Ok(None);
                }
                let v = self.resolve_static_field(field.as_ref())?;
                frame.stack.push(v);
            }
            0xb3 => {
                // putstatic
                ...
                let idx = read_u16(code, &mut frame.pc);
                let field = self.resolve_fieldref_cached(cp, idx);
                let cls = field.class_name.as_str();
                let fld = field.member_name.clone();
                let desc = field.descriptor.as_str();
                if self.ensure_class_init_or_schedule(cls)? {
                    frame.pc = opcode_pc;
                    return Ok(None);
                }
                let val = frame.stack.pop().unwrap_or(JValue::Void);
                if let Some(owner_id) = self.static_field_owner_id_or_self(cls, &fld, desc) {
                    self.set_static_field_value_by_id(owner_id, fld, val);
                }
            }
            0xb4 => {
                // getfield
                let idx = read_u16(code, &mut frame.pc);
                let field = self.resolve_fieldref_cached(cp, idx);
                let gf_field_name = field.member_name.as_str();
                let obj_ref = frame.stack.pop().ok_or_else(|| {
                    format!("getfield {gf_field_name}: empty stack in {class_name}")
                })?;
                ...
                let v = self.resolve_instance_field(field.as_ref(), &obj_ref)?;
                frame.stack.push(v);
            }
            0xb5 => {
                // putfield
                ...
                if matches!(obj_ref, JValue::Void) {
                    let field = self.resolve_fieldref_cached(cp, idx);
                    let pf_field_name = field.member_name.as_str();
                    return Err(format!(
                        "putfield {pf_field_name}: expected Ref on stack, got Void in {class_name}"
                    ));
                }
                let field = self.resolve_fieldref_cached(cp, idx);
                self.set_instance_field(field.as_ref(), &obj_ref, val)?;
            }
            ...
            0xba => {
                ...
                let result =
                    self.dispatch_invokedynamic(cp, idx, frame, class_name, bootstrap_methods)?;
            ...
            0xbb => {
                ...
                let opcode_pc = frame.pc - 1;
                let idx = read_u16(code, &mut frame.pc);
                let new_class = self.resolve_classref_cached(cp, idx);
                if self.ensure_class_init_or_schedule(new_class.as_ref())? {
                    frame.pc = opcode_pc;
                    return Ok(None);
                }
                ...
                if self.class_parse_error(new_class.as_ref()).is_some() {
                    self.throw_class_format_error(new_class.as_ref());
                    return Err(format!(
                        "java/lang/ClassFormatError: malformed class file for {}",
                        new_class
                    ));
                }
                let obj = if self.get_class(new_class.as_ref()).is_some() {
                    // Class is loaded (bytecode available) — use plain object.
                    let field_capacity = self.instance_field_capacity(new_class.as_ref());
                    let field_slots = self
                        .instance_field_layout(new_class.as_ref())
                        .default_values
                        .as_ref()
                        .clone();
                    if let Some(class_id) = self.tracked_class_id_for_name(new_class.as_ref()) {
                        self.allocate_loaded_object(class_id, field_capacity, field_slots)
                    } else {
                        JObject::with_field_capacity_and_slots(
                            new_class.to_string(),
                            field_capacity,
                            field_slots,
                        )
                    ...
                    match new_class.as_ref() {
                        // JDK collection types backed by Array payload (no shim loaded).
                        "java/util/ArrayList" | "java/util/LinkedList" => {
                            JObject::new_array(new_class.to_string(), vec![])
                        }
                        _ => JObject::new(new_class.to_string()),
                    }
                };
            ...
            0xbd => {
                // anewarray
                let idx = read_u16(code, &mut frame.pc);
                let elem_class = self.resolve_classref_cached(cp, idx);
                let count_int = frame.stack.pop().unwrap().as_int();
                if count_int < 0 {
                    return Err(format!("java/lang/NegativeArraySizeException: {count_int}"));
                }
            ...
            0xc5 => {
                ...
                frame.pc += 1;
                let class_name_str = self.resolve_classref_cached(cp, idx);
                ...
                dim_sizes.reverse();
                let arr = self.create_multi_array(class_name_str.as_ref(), &dim_sizes, 0);
                frame.stack.push(JValue::Ref(Some(arr)));
            }
            0xbe => {
                // arraylength
            ...
            0xc0 => {
                // checkcast — per JVMS §6.5.checkcast
                let idx = read_u16(code, &mut frame.pc);
                let target_class = self.resolve_classref_cached(cp, idx);
                ...
                match obj.as_ref() {
                    ...
                    Some(r) => {
                        ...
                        if !self.is_instance_of_object(r, target_class.as_ref()) {
                            return Err(format!(
            ...
            0xc1 => {
                ...
                let is_instance = match obj.as_ref() {
                    None => false,
                    Some(r) => self.is_instance_of_object(r, target_class.as_ref()),
                };
            ...
            // ---- wide prefix ----
            0xc4 => {
                let sub = code[frame.pc];
                frame.pc += 1;
                let local_idx = read_u16(code, &mut frame.pc) as usize;
                match sub {
                    0x15 | 0x16 | 0x17 | 0x18 | 0x19 => {
                        frame.stack.push(frame.locals[local_idx].clone());
                    }
                    0x36 | 0x37 | 0x38 | 0x39 | 0x3a => {
                        let v = frame.stack.pop().unwrap();
                        frame.locals[local_idx] = v;
                    }
                    0x84 => {
                        let c = read_i16(code, &mut frame.pc);
                        if let JValue::Int(ref mut v) = frame.locals[local_idx] {
                            *v = v.wrapping_add(c as i32);
                        }
                    }
            ...
            0xc2 => {
                ...
                match obj_val {
                    ...
                    JValue::Ref(None) => {
                        return Err(
                            "java/lang/NullPointerException: monitorenter on null".to_owned()
                        )
                    }
                    _ => {
                        return Err(
                            "Internal VM error: monitorenter on non-reference value".to_owned()
                        )
                    }
                }
            }
            0xc3 => {
                ...
                match obj_val {
                    ...
                    JValue::Ref(None) => {
                        return Err("java/lang/NullPointerException: monitorexit on null".to_owned())
                    }
                    _ => {
                        return Err(
                            "Internal VM error: monitorexit on non-reference value".to_owned()
                        )
                    }
                }
            }
    ...
    fn resolve_static_field(&mut self, field: &ResolvedMemberRef) -> Result<JValue, String> {
        let class_name = field.class_name.as_str();
        let field_name = field.member_name.as_str();
        let descriptor = field.descriptor.as_str();
        ...
        let owner_id = self.static_field_owner_id_or_self(class_name, field_name, descriptor);
        if let Some(owner_id) = owner_id {
            if let Some(v) = self.static_field_value_by_id(owner_id, field_name) {
                return Ok(v.clone());
            }
        }
        ...
        let owner_name = owner_id
            .and_then(|owner_id| self.class_name(owner_id))
            .unwrap_or(class_name);
        match (owner_name, field_name) {
            ("java/lang/System", "out") => {
                if let Some(v) = self.static_field_value("java/lang/System", "out") {
                    return Ok(v.clone());
                }
                let v = JValue::Ref(Some(JObject::new_print_stream(false)));
                self.set_static_field_value("java/lang/System", field_name.to_owned(), v.clone());
                Ok(v)
            }
            ("java/lang/System", "err") => {
                if let Some(v) = self.static_field_value("java/lang/System", "err") {
                    return Ok(v.clone());
                }
            ...
            ("java/lang/System", "in") => {
                if let Some(v) = self.static_field_value("java/lang/System", "in") {
    ...
    fn resolve_instance_field(
        &mut self,
        field: &ResolvedMemberRef,
        obj_ref: &JValue,
    ) -> Result<JValue, String> {
        let field_name = field.member_name.as_str();
        let field_desc = field.descriptor.as_str();
        match obj_ref.as_ref() {
            Some(r) => {
                let borrow = r.borrow();
                if let Some(slot) = field.instance_field_slot {
                    if let Some(value) = borrow.field_slots.get(slot).cloned() {
                        return Ok(value);
                    }
                }
                let default = default_value_for_descriptor(field_desc);
                Ok(borrow.fields.get(field_name).cloned().unwrap_or(default))
            }
            ...
            Some(r) => {
                ...
                if let Some(slot) = field.instance_field_slot {
                    if let Some(slot_value) = borrow.field_slots.get_mut(slot) {
                        *slot_value = val.clone();
                    }
                }
                borrow.fields.insert(field_name.to_owned(), val);
                Ok(())
            }
