...
impl Vm {
    ...
    pub(crate) fn execute_opcode(
        ...
        match opcode {
            ...
            0xbb => {
                ...
                if matches!(
                    self.classes.get(&new_class),
                    Some(super::LazyClass::ParseError(_))
                ) {
                    self.throw_class_format_error(&new_class);
                    return Err(format!(
                        "java/lang/ClassFormatError: malformed class file for {new_class}"
                    ));
                }
                let obj = if self.get_class(&new_class).is_some() {
                    // Class is loaded (bytecode available) — use plain object.
                    let field_capacity = self.instance_field_capacity(&new_class);
                    JObject::with_field_capacity(new_class, field_capacity)
                } else {
            ...
            0xbd => {
                ...
                let count = count_int as usize;
                let arr =
                    JObject::new_array(format!("[L{elem_class};"), vec![JValue::Ref(None); count]);
                frame.stack.push(JValue::Ref(Some(arr)));
            }
            0xc5 => {
                // multianewarray
    ...
    fn resolve_static_field(&mut self, field: &ResolvedMemberRef) -> Result<JValue, String> {
        ...
        match (owner.as_deref().unwrap_or(class_name), field_name) {
            ("java/lang/System", "out") => {
                ...
                self.static_fields
                    ...
                    .or_default()
                    .insert(field_name.to_owned(), v.clone());
                Ok(v)
            }
    ...
    fn set_instance_field(
        ...
        match obj_ref.as_ref() {
            Some(r) => {
                r.borrow_mut().fields.insert(field_name.to_owned(), val);
                Ok(())
            }
