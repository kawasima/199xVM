...
// 199xVM: jdk.internal.math.DoubleConsts replaced with inline constants
// 199xVM: jdk.internal.math.FormattedFPDecimal replaced with String.format fallback
// 199xVM: sun.util.locale.provider removed (locale zero digit defaults to '0')
...
public final class Formatter implements Closeable, Flushable {
    ...
    // 199xVM: inline constants replacing jdk.internal.math.DoubleConsts
    ...
    // 199xVM: replacement for jdk.internal.math.FormattedFPDecimal
    // Uses BigDecimal to decompose a double into mantissa + exponent strings.
    private static final class FormattedFPDecimal {
        static final int SCIENTIFIC = 0;
        static final int PLAIN = 1;
        static final int GENERAL = 2;

        private final char[] mantissa;
        private final char[] exponent;
        private final int exponentRounded;

        private FormattedFPDecimal(char[] mantissa, char[] exponent, int exponentRounded) {
            this.mantissa = mantissa;
            this.exponent = exponent;
            this.exponentRounded = exponentRounded;
        }

        char[] getMantissa() { return mantissa; }
        char[] getExponent() { return exponent; }
        int getExponentRounded() { return exponentRounded; }

        static FormattedFPDecimal valueOf(double v, int precision, int form) {
            if (v == 0.0) {
                StringBuilder m = new StringBuilder("0");
                if (precision > 0) {
                    m.append('.');
                    for (int i = 0; i < precision; i++) m.append('0');
                }
                return new FormattedFPDecimal(m.toString().toCharArray(),
                    new char[]{'+', '0', '0'}, 0);
            }
            java.math.BigDecimal bd = java.math.BigDecimal.valueOf(v);
            switch (form) {
                case SCIENTIFIC: {
                    // Normalize to a.bbb x 10^e form
                    bd = bd.stripTrailingZeros();
                    int exp10 = bd.precision() - bd.scale() - 1;
                    java.math.BigDecimal norm = bd.movePointLeft(exp10)
                        .setScale(precision, java.math.RoundingMode.HALF_UP).abs();
                    String mStr = norm.toPlainString();
                    String eStr = (exp10 >= 0 ? "+" : "-") + String.format("%02d", Math.abs(exp10));
                    return new FormattedFPDecimal(mStr.toCharArray(), eStr.toCharArray(), exp10);
                }
                case PLAIN: {
                    bd = bd.setScale(precision, java.math.RoundingMode.HALF_UP).abs();
                    return new FormattedFPDecimal(bd.toPlainString().toCharArray(), null, 0);
                }
                case GENERAL: {
                    bd = bd.stripTrailingZeros();
                    int exp10 = bd.precision() - bd.scale() - 1;
                    if (exp10 < -4 || exp10 >= precision) {
                        // Scientific form
                        java.math.BigDecimal norm = bd.movePointLeft(exp10)
                            .setScale(precision - 1, java.math.RoundingMode.HALF_UP).abs();
                        String mStr = norm.toPlainString();
                        String eStr = (exp10 >= 0 ? "+" : "-") + String.format("%02d", Math.abs(exp10));
                        return new FormattedFPDecimal(mStr.toCharArray(), eStr.toCharArray(), exp10);
                    } else {
                        // Decimal form
                        int scale = precision - exp10 - 1;
                        if (scale < 0) scale = 0;
                        bd = java.math.BigDecimal.valueOf(v).setScale(scale, java.math.RoundingMode.HALF_UP).abs();
                        return new FormattedFPDecimal(bd.toPlainString().toCharArray(), null, exp10);
                    }
                }
                default:
    ...
    static class FormatSpecifier implements FormatString {
        ...
        private String hexDouble(double d, int prec) {
            ...
            if (!Double.isFinite(d) || d == 0.0 || prec == 0 || prec >= 13) {
                ...
                int shiftDistance
                    =  SIGNIFICAND_WIDTH - precision;
                assert(shiftDistance >= 1 && shiftDistance < SIGNIFICAND_WIDTH);
                ...
                long signBit = doppel & SIGN_BIT_MASK;
            ...
            if (Flags.contains(f, Flags.GROUP)) {
                ...
                if (l == null || l.equals(Locale.US)) {
                    ...
                    if (nf instanceof DecimalFormat) {
                        ...
                        // 199xVM: simplified — use default DecimalFormat
