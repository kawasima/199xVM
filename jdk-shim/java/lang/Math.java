...
public final class Math {
    ...
    public static int round(float a) {
        ...
        int biasedExp = (intBits & 0x7F800000)
                >> (24 - 1);
        int shift = (24 - 2
                + 127) - biasedExp;
        if ((shift & -32) == 0) { // shift >= 0 && shift < 32
            // a is a finite number such that pow(2,-32) <= ulp(a) < 1
            int r = ((intBits & 0x007FFFFF)
                    | (0x007FFFFF + 1));
            if (intBits < 0) {
                r = -r;
            }
            ...
            // a is either
            // - a finite number with abs(a) < exp(2,24-32) < 1/2
    ...
    public static long round(double a) {
        ...
        long biasedExp = (longBits & 0x7FF0000000000000L)
                >> (53 - 1);
        long shift = (53 - 2
                + 1023) - biasedExp;
        if ((shift & -64) == 0) { // shift >= 0 && shift < 64
            // a is a finite number such that pow(2,-64) <= ulp(a) < 1
            long r = ((longBits & 0x000FFFFFFFFFFFFFL)
                    | (0x000FFFFFFFFFFFFFL + 1));
            if (longBits < 0) {
                r = -r;
            }
            // In the comments below each Java expression evaluates to the value
            ...
            // a is either
            // - a finite number with abs(a) < exp(2,53-64) < 1/2
            // - a finite number with ulp(a) >= 1 and hence a is a mathematical integer
            // - an infinity or NaN
            return (long) a;
        }
    ...
    private static final long negativeZeroFloatBits  = 0x80000000L;       // Float.floatToRawIntBits(-0.0f)
    private static final long negativeZeroDoubleBits = 0x8000000000000000L; // Double.doubleToRawLongBits(-0.0d)
    ...
    public static int getExponent(float f) {
         ...
         */
        return ((Float.floatToRawIntBits(f) & 0x7F800000) >>
                (24 - 1)) - 127;
    }
    ...
    public static int getExponent(double d) {
         ...
         * get true exponent value.
         */
        return (int)(((Double.doubleToRawLongBits(d) & 0x7FF0000000000000L) >>
                      (53 - 1)) - 1023);
    }
    ...
    private static final double F_UP = 0x1p1023;  // normal, exact, 2^1023
    private static final double F_DOWN = 0x1p-1023;  // subnormal, exact, 2^-1023
    ...
    public static double scalb(double d, int scaleFactor) {
        if (scaleFactor > -1023) {
            ...
            if (scaleFactor <= 2 * 1023) {
                return d * primPowerOfTwoD(scaleFactor - 1023) * F_UP;
            }
            if (scaleFactor < 2 * 1023 + PRECISION - 1) {
                return d * primPowerOfTwoD(scaleFactor - 2 * 1023) * F_UP * F_UP;
            }
            return d * F_UP * F_UP * F_UP;
        }
        if (scaleFactor > -2 * 1023) {
            return d * primPowerOfTwoD(scaleFactor + 1023) * F_DOWN;
        }
        if (scaleFactor > -2 * 1023 - PRECISION) {
            return d * primPowerOfTwoD(scaleFactor + 2 * 1023) * F_DOWN * F_DOWN;
        }
    ...
    private static double primPowerOfTwoD(int n) {
        return longBitsToDouble((long) (n + 1023) << PRECISION - 1);
    }
