...
public class BigInteger extends Number implements Comparable<BigInteger> {
    ...
    */
    ...
    final int compareMagnitude(BigInteger val) {
        ...
        if (len1 > len2)
            return 1;
        int i = mismatch(m1, m2, len1);
        if (i != -1)
            return Integer.compareUnsigned(m1[i], m2[i]) < 0 ? -1 : 1;
        return 0;
    }

    private static int mismatch(int[] a, int[] b, int len) {
        for (int i = 0; i < len; i++) {
            if (a[i] != b[i]) {
                return i;
            }
    ...
    private static int hashCode(int[] a, int from, int to, int initial) {
        int result = initial;
    ...
    public boolean equals(Object x) {
        ...
        return mismatch(mag, xInt.mag, mag.length) == -1;
    }
    ...
    @Override
    public int hashCode() {
        return hashCode(mag, 0, mag.length, 0) * signum;
    }
    ...
    public float floatValue() {
         ...
         */
        int shift = exponent - FLOAT_SIGNIFICAND_WIDTH;
        ...
        signifFloor &= FLOAT_SIGNIF_BIT_MASK; // remove the implied bit
        ...
        int bits = ((exponent + FLOAT_EXP_BIAS))
         ...
         */
        bits |= signum & FLOAT_SIGN_BIT_MASK;
    ...
    public double doubleValue() {
         ...
         * 2^(SIGNIFICAND_WIDTH - 1 - exponent).
         */
        int shift = exponent - DOUBLE_SIGNIFICAND_WIDTH;
        ...
        signifFloor &= DOUBLE_SIGNIF_BIT_MASK; // remove the implied bit
        ...
        long bits = (long) ((exponent + DOUBLE_EXP_BIAS))
         ...
         */
        bits |= signum & DOUBLE_SIGN_BIT_MASK;
