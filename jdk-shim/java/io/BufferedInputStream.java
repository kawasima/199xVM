...
public class BufferedInputStream extends FilterInputStream {
    private static int newLength(int old, int min, int pref) { return Math.max(old + min, old + pref); }
     ...
     */
    // 199xVM: Unsafe CAS replaced with synchronized method
    private synchronized boolean casBuf(byte[] expected, byte[] update) {
        if (buf == expected) {
            buf = update;
            return true;
        }
