    ...
    implements Comparable<URI>, Serializable
{
    ...
    // Encode a single non-ASCII char to UTF-8 percent-escape sequences
    private static void appendEncoded(StringBuilder sb, char c) {
        byte[] bytes = String.valueOf(c).getBytes("UTF-8");
        for (byte b : bytes) {
            int v = b & 0xff;
            if (v >= 0x80)
                appendEscape(sb, (byte) v);
            else
                sb.append((char) v);
        }
    }
    ...
    private static String decode(String s, boolean ignorePercentInBrackets) {
        ...
        for (int i = 0; i < n;) {
            ...
            for (;;) {
                assert (n - i >= 2);
                bb[numBytes++] = decode(s.charAt(++i), s.charAt(++i));
                if (++i >= n)
                    break;
                c = s.charAt(i);
                if (c != '%')
            ...
            sb.append(new String(bb, 0, numBytes, "UTF-8"));
        }
